  ...
  return {
    async createPostWithTargets({ post, targetRows }) {
      ...
      const createdTargets = targetRows.length
        ? await db.insert(postTargets).values(targetRows.map((target) => ({ ...target, postId: createdPost.id }))).returning()
        : [];
    ...
    async cancelScheduledPost(ownerEmail, postId, now) {
      const [updated] = await db.update(posts).set({ status: POST_STATUS.CANCELLED, updatedAt: now })
        .where(and(eq(posts.ownerEmail, ownerEmail), eq(posts.id, postId), eq(posts.status, POST_STATUS.SCHEDULED))).returning();
      if (!updated) return null;
      await db.update(postTargets).set({ status: POST_STATUS.CANCELLED, updatedAt: now }).where(eq(postTargets.postId, postId));
      return this.findPostByOwner(ownerEmail, postId);
    },
    async claimPostForPublish(ownerEmail, postId, now) {
      const [updated] = await db.update(posts).set({ status: POST_STATUS.PUBLISHING, publishingStartedAt: now, updatedAt: now })
        .where(and(eq(posts.ownerEmail, ownerEmail), eq(posts.id, postId), eq(posts.status, POST_STATUS.DRAFT))).returning();
      if (!updated) return null;
      await db.update(postTargets).set({ status: POST_STATUS.PUBLISHING, updatedAt: now }).where(eq(postTargets.postId, postId));
      return this.findPostByOwner(ownerEmail, postId);
    },
    async recordPublishResults(ownerEmail, postId, results, now) {
      ...
      const current = await this.findPostByOwner(ownerEmail, postId);
      const status = resolvePostStatus(current.targets);
      await db.update(posts).set({ status, publishedAt: status === POST_STATUS.PUBLISHED ? now : null, updatedAt: now })
...
async function attachTargets(db, postRows) {
  ...
  const targets = await db.select().from(postTargets).where(inArray(postTargets.postId, postRows.map((post) => post.id)));
