...
const handlers = createSupportHumanActionRouteHandlers({ requireOwner: requireSettingsAccess, requireSameOrigin, getStore: getSupportStore, startTransition: async (input) => { const { supportTransitionWorkflow } = await import("../../../../../../lib/support/workflows/support-transition-workflow.js"); return start(supportTransitionWorkflow, [input]); }, respond: (body, init) => NextResponse.json(body, init) });
