...
- Retryable lifecycle or publishing failures atomically queue both immediate and scheduled work for the next strictly-future deployed scheduler occurrence, currently daily at `01:00 UTC` (`0 1 * * *` in `vercel.json`). An immediate request returns HTTP 202 with the sanitized queued post, so the browser does not resubmit and risk duplication. Published and permanently failed targets are persisted before retryable targets are rescheduled, and only explicitly retryable targets are claimed again.
- Deployments requiring shorter retry latency must configure a scheduler frequency supported by their Vercel plan and align `retry-schedule.js`; plan and cost authorization remain operator-owned rather than an application-code assumption.
- Provider calls use bounded deadlines shorter than renewal leases, and the deadline remains active through response-body consumption for lifecycle, publish, and LINE revoke requests. Lease completion is fenced: a worker that loses its compare-and-set observes demonstrably newer credentials or returns a typed retryable error, never stale credentials.
- Provider 400/404/422 validation or media failures are terminal for that target without mutating connection state. Network/abort, ambiguous 403, 429, and 5xx are retryable. Unknown connection-storage exceptions also retry; only explicit domain outcomes and safely wrapped credential-integrity failures terminalize.
