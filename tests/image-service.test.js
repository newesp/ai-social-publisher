...
test("generates Gemini images with the interactions endpoint payload", async () => {
  ...
  const result = await generateGeminiImage({
    ...
    fetchImpl: async (url, options) => {
      ...
      return {
        ok: true,
        json: async () => ({ output_image: { data: "base64-image", mime_type: "image/jpeg" } }),
        text: async () => "",
      };
    },
  });

  assert.equal(result, "data:image/jpeg;base64,base64-image");
...
test("uses the requested Gemini image model", async () => {
  ...
  await generateGeminiImage({
    ...
    fetchImpl: async (_url, options) => {
      calls.push(options);
      return { ok: true, json: async () => ({ output_image: { data: "base64-image", mime_type: "image/jpeg" } }), text: async () => "" };
    },
  });
...
test("returns the documented top-level Gemini interaction image output", async () => {
  const result = await generateGeminiImage({
    prompt: "Create a product image",
    settings: { googleAiApiKey: "google-key" },
    fetchImpl: async () => ({
      ok: true,
      json: async () => ({ output_image: { data: "top-level-base64-image", mime_type: "image/jpeg" } }),
      text: async () => "",
    }),
  });

  assert.equal(result, "data:image/jpeg;base64,top-level-base64-image");
});
