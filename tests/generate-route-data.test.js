...
test("builds generated response with editable targets and uploaded image URL", async () => {
  const response = await buildGeneratedResponse({
    ...
    generateTargets: async ({ llmModel }) => {
      assert.equal(llmModel, "gemini-3.5-flash");
      return [
    ...
    generateImage: async ({ imageModel }) => {
      assert.equal(imageModel, "gemini-3.1-flash-image");
  ...
  assert.equal(response.llmModel, "gemini-3.5-flash");
  assert.equal(response.imageModel, "gemini-3.1-flash-image");
